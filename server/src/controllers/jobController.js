...
exports.updateJob = async (req, res) => {
    try {
        const { id } = req.params;
        const recruiterId = req.user.id;
        const updateData = req.body; // Lấy tất cả dữ liệu gửi lên
        ...
        // --- Xây dựng object chỉ chứa các trường hợp lệ cần cập nhật ---
        const allowedFields = ['title', 'description', 'requirements', 'salary', 'location', 'status', 'application_limit', 'company_id'];
        const fieldsToUpdate = {};
        let hasValidField = false; // Cờ kiểm tra xem có trường nào hợp lệ không

        for (const key in updateData) {
            if (allowedFields.includes(key)) {
                // Xử lý application_limit riêng
                if (key === 'application_limit') {
                     const limit = (updateData[key] && !isNaN(parseInt(updateData[key], 10)) && parseInt(updateData[key], 10) > 0)
                                   ? parseInt(updateData[key], 10)
                                   : null;
                     fieldsToUpdate[key] = limit;
                }
                // Xử lý company_id riêng (chuyển sang số)
                else if (key === 'company_id') {
                    fieldsToUpdate[key] = parseInt(updateData[key], 10) || null; // Chuyển sang số hoặc null
                }
        ...
        // --- Kết thúc xây dựng object ---

        // Nếu không có trường nào hợp lệ được gửi lên thì báo lỗi
        if (!hasValidField) {
             return res.status(400).json({ message: 'Không có dữ liệu hợp lệ để cập nhật.' });
        }

        // Thực hiện cập nhật chỉ với các trường hợp lệ
        await db.query('UPDATE jobs SET ? WHERE id = ?', [fieldsToUpdate, id]);

        res.status(200).json({ message: 'Cập nhật tin tuyển dụng thành công.' });
    } catch (error) {
        // Kiểm tra lỗi nếu title bị cố tình đặt thành null (dù frontend nên validate)
        if (error.code === 'ER_BAD_NULL_ERROR' && error.sqlMessage.includes("'title'")) {
             return res.status(400).json({ message: 'Tiêu đề không được để trống.' });
        }
        console.error("Error in updateJob:", error);
        res.status(500).json({ message: 'Lỗi server khi cập nhật việc làm' });
    }
};
