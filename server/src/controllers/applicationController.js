...
// @route   GET /api/applications/:jobId getJobApplications
...
exports.getJobApplications = async (req, res) => {
    try {
        ...
        // 1. Lấy thông tin job và kiểm tra quyền (giữ nguyên)
        const [jobs] = await db.query('SELECT recruiter_id, status as jobStatus FROM jobs WHERE id = ?', [jobId]);
        if (jobs.length === 0) {
        ...
        const jobStatus = jobs[0].jobStatus;

        // 2. Lấy danh sách ứng viên (SỬA LẠI CÂU QUERY Ở ĐÂY)
        // Thay thế '...' bằng các cột cụ thể
        const [applications] = await db.query(
            `SELECT
             ...
             LEFT JOIN (
                -- Subquery lấy interview mới nhất
             ...
             LEFT JOIN (
                -- Subquery lấy offer mới nhất (nếu có thể có nhiều)
                SELECT application_id, status, ROW_NUMBER() OVER(PARTITION BY application_id ORDER BY created_at DESC) as rn
                FROM offers
             ) o ON a.id = o.application_id AND o.rn = 1
             WHERE a.job_id = ?
             ORDER BY a.applied_at DESC`,
            [jobId]
        );

        // 3. Trả về kết quả (giữ nguyên)
        res.status(200).json({
            jobStatus: jobStatus,
        ...
        console.error("Error in getJobApplicants:", error); // Log lỗi chi tiết hơn
