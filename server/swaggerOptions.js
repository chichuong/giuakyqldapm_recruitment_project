...
const swaggerDefinition = {
  ...
  components: {
    ...
    schemas: {
      ...
      // NEW SCHEMA FOR ADMIN USER UPDATE
      UserProfileUpdateAdmin: {
        type: 'object',
        properties: {
          full_name: { type: 'string', example: 'Nguyễn Văn B' },
          email: { type: 'string', format: 'email', example: 'nguyen.van.b@admin.com' },
          phone_number: { type: 'string', nullable: true },
          role: { type: 'string', enum: ['candidate', 'recruiter', 'admin'], example: 'recruiter' },
          headline: { type: 'string', nullable: true },
          bio: { type: 'string', nullable: true },
          skills: { type: 'string', nullable: true, description: 'Kỹ năng (cách nhau bởi dấu phẩy).' },
          linkedin_url: { type: 'string', format: 'url', nullable: true },
          github_url: { type: 'string', format: 'url', nullable: true },
        },
      },
      UserChangePassword: {
        type: 'object',
      ...
      // NEW SCHEMA FOR ADMIN COMPANY UPDATE
      CompanyUpdateAdmin: {
        type: 'object',
        required: ['name'],
        properties: {
          name: { type: 'string', example: 'Công ty Cập Nhật' },
          description: { type: 'string', nullable: true, example: 'Mô tả mới nhất.' },
          address: { type: 'string', nullable: true, example: 'Địa chỉ mới.' },
          logo_url: { type: 'string', format: 'url', nullable: true, description: 'URL logo mới.' },
        },
      },
      ApplicationApply: {
        type: 'object',
  ...
  paths: {
    ...
    // ------------------------- ADMIN ROUTES (MỚI: CRUD) -------------------------
    '/api/admin/users/{userId}': {
      parameters: [
        { in: 'path', name: 'userId', required: true, schema: { type: 'integer' }, description: 'ID người dùng.' },
      ],
      put: {
        tags: ['Admin'],
        summary: '[Admin] Cập nhật thông tin chi tiết người dùng',
        security: [{ bearerAuth: [] }],
        requestBody: {
          required: true,
          content: { 'application/json': { schema: { $ref: '#/components/schemas/UserProfileUpdateAdmin' } } },
        },
        responses: {
          '200': { description: 'Cập nhật thành công.' },
          '400': { description: 'Dữ liệu không hợp lệ hoặc không thể tự thay đổi vai trò.' },
          '403': { description: 'Yêu cầu vai trò Admin.' },
      ...
      delete: {
        ...
        responses: {
          '200': { description: 'Xóa thành công.' },
          '400': { description: 'Không thể tự xóa hoặc có ràng buộc dữ liệu.' },
          '403': { description: 'Yêu cầu vai trò Admin.' },
          '404': { description: 'Không tìm thấy người dùng.' },
        },
      },
    ...
    '/api/admin/companies/{companyId}': {
      parameters: [
        { in: 'path', name: 'companyId', required: true, schema: { type: 'integer' }, description: 'ID công ty.' },
      ],
      put: {
        tags: ['Admin'],
        ...
        requestBody: {
          required: true,
          content: { 'application/json': { schema: { $ref: '#/components/schemas/CompanyUpdateAdmin' } } },
        },
        responses: {
          '200': { description: 'Cập nhật thành công.' },
          '400': { description: 'Tên công ty là bắt buộc.' },
          '403': { description: 'Yêu cầu vai trò Admin.' },
      ...
      delete: {
        tags: ['Admin'],
        summary: '[Admin] Xóa công ty (cập nhật các job liên quan thành NULL)',
        security: [{ bearerAuth: [] }],
        responses: {
          '200': { description: 'Xóa công ty thành công.' },
          '400': { description: 'Có ràng buộc dữ liệu.' },
          '403': { description: 'Yêu cầu vai trò Admin.' },
          '404': { description: 'Không tìm thấy công ty.' },
        },
      },
